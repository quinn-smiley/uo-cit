...
// Create database connection pool (better for handling multiple requests)
...
const users = [];
const players = []; // Keep for backward compatibility
...
// Legacy /name endpoint (for backward compatibility)
app.put("/profile/name", async (req, res) => {
  ...
  try {
    ...
    // Update session so the change is reflected immediately
...
app.post("/responses", async (req, res) => {
  ...
  // Validate answers as strings against allowed options
  ...
  // Save text in session for frontend display
...
// Get the result of the most recent quiz attempt for the current session
app.get("/get-result", async (req, res) => {
  ...
  try {
    // Fetch the user's responses for this attempt
    ...
    // Compute result based on same logic as procedure
    let quizResult;
    ...
    // Save result in session for frontend display
